...
- `findings/YYYY-MM-DD/pins.jsonl` — Layer 2's proposed pins, one JSON object per line (`project`, `title`, `body`, `kind`). A file left by an earlier run or attempt is moved to `pins.jsonl.stale-<epoch>-<attempt>` before each L2 attempt.
- `findings/YYYY-MM-DD/pin-projects.tsv` — `project<TAB>cwd` rows for every project in this run's worklist (`sessions.txt`), written after a complete report. Project and cwd both come from the real session path, never from a findings JSON, whose `session_path` the L1 model writes; `bin/apply-pins.sh` reads it to authorize and resolve each pin.
