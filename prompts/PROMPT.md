...
- **A purely behavioral finding** (`compliance_failure`, `tool_loop`, a missed marker) has no artifact to re-check. Its transcript evidence stands as-is and this gate does not apply.
...
- **Corpus integrity**: these keys say whether the night's corpus is the whole corpus, and every one of them is silent when it reads zero. `roots_failed` counts session roots that were reached but whose enumeration failed and returned nothing — each one is a store contributing no sessions tonight, so name it and say the corpus is short by an unknown amount. `roots_partially_enumerated` is the milder form: the walk returned data but exited nonzero, usually one unreadable directory, so the count for that root is a floor rather than a total. `roots_unavailable` counts configured roots that were not directories at all, which usually means a `:` in a path (`SESSION_ROOTS` is colon-separated) or a store that moved. `sessions_rejected_path` counts sessions dropped for holding a character the artifact list or the L1 fan-out cannot carry (newline, tab, backslash, quote — see #54); these never reach the worklist, so they are missing from `sessions_found_raw` too. `sessions_duplicate_path` counts transcripts reached more than once, ordinarily because two entries of `SESSION_ROOTS` overlap (one directory a symlink of another) — harmless, but a large number means the root list is redundant and worth pruning. `sessions_hash_collision` and `sessions_dropped_to_collision` are the artifact-key collision counters: the first counts events, the second counts paths, and each event drops at least two paths, so never add them together. `sidecar_stale_rows` reads `unknown` when the check itself broke rather than when nothing was stale. `adapters_rejected` names adapter directories the loader refused (`none` is the healthy reading, and a blank value means the key was never measured); `adapters_enabled` names the harnesses that were enumerated. `sessions_by_source` counts paths per harness and is **systematically larger than `sessions_triaged`** — it is built before the self-pollution prune and the empty-session filter, so it still carries rows for worker transcripts and 0-turn shells those later steps drop. Read it as a share between harnesses, never as a session count, and do not report the gap against `sessions_triaged` as a discrepancy: on a single-harness night the only honest statement it supports is that every session came from that harness. Report any non-zero value here in one line each, above the per-project sections, because a short corpus changes how every other number in this report should be read. Where they are all zero, say nothing. When a key is absent entirely (findings dirs predating the adapter seam), say nothing.
- **Pipeline capacity**: `l1_findings_with_error` counts sessions that ran to completion but returned empty `findings` with an `error` (transcript too big to fit, even after slimming) — a silent failure that `l1_err_files` (crashed-worker `.err` files) does NOT capture. Compute the extraction-failure rate as `l1_findings_with_error / (sessions_triaged - gated)` — gated sessions never reached a model call, so they cannot contribute an extraction failure and must not inflate the denominator. If that rate is a meaningful fraction, call out the extraction-failure rate and propose the next cc-autodream fix (a `jq` pre-summarizer that strips verbose `tool_result` payloads, a lower `AUTODREAM_SLIM_BYTES`, or a metadata-only fallback pass). When `oversized_total > 0`, also report `oversized_errored / oversized_total` — this is the issue #12 measurement gate (chunk-summarizing oversized transcripts is BLOCKED pending evidence it's needed), and if that errored share sustains >= 5% across a week, flag that issue #12's gate has opened. If `stats_sidecars_unparseable` is non-zero, carry that caveat into this ratio too — some of those sizes were re-measured live rather than read from a sidecar, so note it next to the gate reading rather than presenting the share as clean.
...
- **A night that died**: `fatal: <reason>` in `run-stats.txt` means the run stopped before it triaged anything — it wrote no report and posted no notification, so this line is the only surface that failure has. Name the reason and say plainly that the date produced nothing. `no usable adapter` has a mundane cause worth suggesting outright: `adapters/claude/adapter.sh` losing its exec bit to a tarball copy, a restrictive umask or `core.fileMode=false`, since the loader requires `-x`. That failure repeats every night until someone fixes the bit, so treat a `fatal:` on a recent date as urgent rather than historical. When the key is absent, say nothing — that is the normal reading.
