...
| `bin/linearize-omp-session.sh` | OMP (oh-my-pi) source ingest: collapse a session's entry tree to the live leaf chain, root-first, dropping abandoned branches. Emits a leading `type:"autodream_meta"` JSON record (session id, `cwd`, entry/on-path/dropped counts). Fails closed (nonzero, no output) on a non-OMP file, an unparseable line, a cycle, or a dangling `parentId` — the caller must skip the session, never fall back to the raw file. Artifacts only, no model calls. **Not yet wired into `run.sh`** |
