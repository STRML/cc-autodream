...
| `bin/linearize-omp-session.sh` | OMP (oh-my-pi) source ingest: collapse a session's entry tree to the live leaf chain, root-first, dropping abandoned branches. Emits a leading `type:"autodream_meta"` JSON record (session id, `cwd`, entry/on-path/dropped counts, plus `nested`/`parent_session_file` from directory provenance — a child lives in a dir named after its parent session file). Fails closed (nonzero, no output) on a non-OMP file, an unparseable line, a cycle, or a dangling `parentId` — the caller must skip the session, never fall back to the raw file. Artifacts only, no model calls |
| `bin/citation-check.sh` | resolve every 12-hex session citation in a finished report against the findings dir: emits `citations_total` / `citations_unresolved` / `citations_to_gated` (+ `_list` keys) on stdout, which `run.sh` appends to `run-stats.txt` after L2. Catches L2 attaching real content to the wrong session hash — including a noise-gated stub it never received content for. Matches backticked AND bare hashes; biased to false alarm over false all-clear. Never fatal; exit 2 only when the check could not run. Artifacts only, no model calls |
